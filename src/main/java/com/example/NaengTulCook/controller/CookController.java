...
public class CookController {
    ...
    @Operation(summary = "사용자가 필터를 적용하여 요리하기 버튼을 눌렀을때 레시피 및 재료를 반환해줌",
            description = "제외할 재료는 쉼표 기준으로 연결해서 보내주세요. 나머지 필터들은 선택한 버튼 텍스트 그대로 보내주시면 됩니다.",
            responses = {
                    @ApiResponse(
                            responseCode = "200",
                            ...
                            content = @Content(
                                    ...
                                    schema = @Schema(
                                            ...
                                            example = "[{\"seasoning\": [\"소금\", \"후추\", \"우스터 소스\", \"간장\"], \"newPlating\": [\"1. 야끼소바를 그릇에 담고, 양배추를 곁들여주세요.\", \"2. 고명이나 깨를 뿌려서 마무리해주세요.\", \"3. 야끼소바를 만들어 먹으면 케토 다이어트에도 적합하고 저칼로리 식사를 즐길 수 있습니다.\"], \"ingredients\": [\"소바면\", \"우스터 소스\", \"양배추\", \"돼지고기\", \"숙주\"], \"newCook\": [\"1. 먼저 돼지고기와 숙주를 굽거나 볶아주세요.\", \"2. 소바면을 삶아서 물기를 제거해주세요.\", \"3. 양배추를 채썰어 준비해주세요.\", \"4. 우스터 소스를 소바면에 골고루 섞어주세요.\", \"5. 모든 재료를 섞어 완성된 야끼소바를 만들어주세요.\"], \"newSeasoning\": [\"1. 우스터 소스를 조금 더 추가해서 짭짤한 맛을 살려주세요.\", \"2. 소금이나 간장을 이용해서 간을 조절해주세요.\"], \"tool\": [\"프라이팬\", \"젓가락\"]}]"
                                    )
                            )
                    ),
                    @ApiResponse(
    ...
    @Operation(summary = "사용자가 필터를 적용하지 않고 요리하기를 눌렀을 때 레시피 반환",
            description = "레시피들을 문장 사이에 \\n을 넣어놨으니 그거 기준으로 쪼개서 줄바꿈 해주시면 될 것 같습니다",
            responses = {
                    @ApiResponse(
                            responseCode = "200",
                            description = "Recipe created successfully",
                            content = @Content(
                                    ...
                                    schema = @Schema(
                                            description = "레시피 반환 결과",
                                            example = "{\"seasoning\": [\"소금\", \"후추\", \"우스터 소스\", \"간장\"], \"recipeCook\": \"   야채(양배추, 당근, 숙주, 파)를 씻고 적당한 크기로 썬다.,\\n    돼지고기는 얇게 썰어 소금과 후추로 밑간을 한다.,\\n    프라이팬을 중불로 달군 후 돼지고기를 넣고 익힌다.,\\n    고기가 익으면 양배추, 당근, 숙주를 넣고 볶는다.,\\n    소바면을 끓는 물에 살짝 데친 후 프라이팬에 넣는다.,\\n    모든 재료를 함께 볶으면서 잘 섞어준다.\", \"recipeSeasoning\": \"    우스터 소스를 넣고 전체적으로 골고루 섞으며 볶는다.,\\n    간장, 소금, 후추를 추가하여 맛을 조절한다.,\\n    약한 불로 줄이고 1~2분간 간이 배도록 익힌다.\", \"recipePlating\": \"    완성된 야끼소바를 접시에 담는다.,\\n    파를 송송 썰어 위에 뿌린다.,\\n    기호에 따라 가쓰오부시나 마요네즈를 곁들인다.\", \"ingredients\": [\"소바면\", \"우스터 소스\", \"양배추\", \"당근\", \"돼지고기\", \"숙주\", \"파\"], \"tool\": [[\"프라이팬\", \"젓가락\"]]}"
                                    )
                            )
                    ),
                    @ApiResponse(
    ...
    public ResponseEntity<?> getRecipeDetails(@RequestParam String recipeName) {
        try {
            ...
            // optional_ingredient를 문자열로 변환 후 split
